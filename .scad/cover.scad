...
aqara_diameter = 85;     // Диаметр накладной розетки
...
module main_case() {
    difference() {
        ...
        translate([
            ...
            wall_thickness + esp32_w/2 - esp32_usb_w/2, 
            0
        ])
            cube([esp32_usb_h + 1, esp32_usb_w, bottom_thickness + 1]);
...
module aqara_rim() {
    difference() {
        ...
        // Вырез для проводов розетки (снизу)
        translate([0, rim_inner_d/2, -0.5])
...
// Обод для крепления на Aqara розетку (раскомментируй для печати)
translate([0, outer_l + 80, 0])
     aqara_rim();
